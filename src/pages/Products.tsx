...
export const Products = () => {
  ...
  const {
    ...
    queryFn: async () => {
      return await getSearchProduct(search);
    },
  });
  ...
  const handleChangeSort = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (event.target.value) {
      return setSearchParams({ ...params, sort: event.target.value });
    }
  ...
  return (
    <>
      <select
        value={sortValue}
        className="fixed m-1 ml-[100px] top-0 h-10 z-10"
      ...
      <div className=" mx-auto max-w-2xl py-3 px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        <div className=" grid grid-cols-1 gap-y-5 gap-x-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
          {sortingProducts(products.data, sort).map((product: CardProducts) => (
            <div key={product._id}>
              <div className="group">
